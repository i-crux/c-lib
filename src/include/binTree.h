...
struct binTreeNodeStruct
{
    ...
    BinTreeNode *left;
    BinTreeNode *right;
    intptr_t property; /* some property for the TreeNode, could using for RBtree, AVLtree */
...
/**
 * @brief left rotate a [BinTreeNode]
 * IMPORTANT: node->right must not be [NULL]
 *
 * @param parent: a pointor of pointor which connect which [node]
 * @param node: node need to be rotated
 */
void binTreeLeftRotate(BinTreeNode **parent, BinTreeNode *node);

/**
 * @brief right rotate a [BinTreeNode]
 * IMPORTANT: node->left must not be [NULL]
 *
 * @param parent: a pointor of pointor which connect which [node]
 * @param node: node need to be rotated
 */
